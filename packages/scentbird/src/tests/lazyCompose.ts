...
function compose(...args: Part<any, any>[]) {
    ...
    for (let i = args.length - 3; i > 0; i--) {
        console.log('compose ', args[i - 1], args[i])
        // @ts-ignore
...
const composeCache = (() => {
        ...
        return (tail, head) => {
            ...
            if (!box) {
                console.log('create new Composition', hash)
                const obj = {
...
function foldInComposition(value) {
    // @ts-ignore
    console.log('calling fold in composition', this, value)
    // @ts-ignore
...
function fold<O, I>(composition: Composition<O, I> | F<O, I>, value: I): O {
    ...
    while (pointer) {
        if (isComposition(pointer)) {
            ...
            console.log('with arg', result)
            console.log('Pointer name', pointer.name)
            result = pointer(result)
            console.log('Pointer result', result)
            pointer = undefined
        }
    }
