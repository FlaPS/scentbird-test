...
/**
 *
 * @param {"react".ComponentType<P>} Taet to render in case of no match
 * @param {Array<Pattern<Partial<P>>>} patterns
 * @returns Wrapped component with conditional rendering
 */
function caseRender<P extends object>( Taet: Renderable<P>, patterns: Pattern<Partial<P>>[] = []): CaseRender<P> {
    if (!patterns.length )
        patterns = [{ matcher: R.T as any as Predicate<P>, Comp: Taet }]


    const matchPredicate = <I>(predicate: Predicate<P & I>, component: Renderable<Partial<P & I>>) =>
        caseRender<P & Partial<I>>(
            Taet as any as Renderable<P & Partial<I>>,
            [...patterns,
                {
                    matcher: predicate,
                    Comp: component,
    ...
    const isNil = <K extends keyof P, N extends Partial<P>>(property: K, component: Renderable<N>) =>
        caseRender<Optional<P, K>>(
            Taet as any as React.ComponentType<Optional<P, K>>,
            [
                ...patterns,
                {
    ...
    Comp.displayName = Taet
        ? 'CaseRender(' + hasDisplayName(Taet) ? Taet['displayName'] : 'Renderable' + ')'
        : 'CaseRender'
