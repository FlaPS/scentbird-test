...
(mapper: {(state: FrontState, props?: P): M}, frontReducer: Reducer) {
    return (Component: React.ComponentType<ForkedChildProps<P, M>>) =>
        connect(
            (state: FrontState) => ({frontState: state}),
            dispatch => ({dispatch}),
        )
        (class extends React.Component< HocProps<P, FrontState>,
                ...
                componentWillReceiveProps(nextProps) {
                    ...
                    if (nextProps.localHistory !== this.props.localHistory) {
                        this.setState({localHistory: this.localHistory as any}, this.saveForkedState)
                    }
                }
