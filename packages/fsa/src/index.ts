...
export type Dispatch<A> =
    | ((value: FactoryAction<A> | FactoryAnyAction ) => void)
    | ((value: FactoryAction<A> | FactoryAnyAction ) => any)
    | ((value: {type: string, payload: A}) => void)
...
export interface ActionCreator<P, F = string> {
    type: string
...
export type EmptyActionCreator<F = string> = (payload?: undefined, meta?: any | null) =>
    ActionCreator<undefined, F>

export type AsyncStatus = 'started' | 'done' | 'failed' | undefined
...
export interface EmptyAsyncActionCreators<S, E, F = string> {
    type: string
    started: EmptyActionCreator<F>
    done: ActionCreator<EmptySuccess<S>>
    failed: ActionCreator<EmptyFailure<E>>
}

export interface ActionCreatorFactory<F = string> {
    (
        prefix: F,
        commonMeta?: any,
        error?: boolean,
    ): EmptyActionCreator<F>
...
export function actionCreatorFactory<F = string>(
    prefix?: F ,
    factoryMeta: {} = {},
    ...
    function asyncActionCreators<P, S, E>(
        ...
        const done = actionCreator<Success<P, S>>(`${type}_DONE`, commonMeta, false)
        const failed = actionCreator<Failure<P, E>>(`${type}_FAILED`, commonMeta, true)
        return {
            type: base + type,
            started,
            done,
